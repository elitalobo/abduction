...
class XGBooster(object):
    ...
    def explain(self, sample, use_lime=False, use_anchor=False, use_shap=False,
            expl_ext=None, prefer_ext=False, nof_feats=5,use_bfs=False):
        """
        ...
        if use_lime:
            ...
            expl = self.x.compute_all_minimal_expls(np.array(sample))
    ...
    def compute_all_explanations(self, feature_ids):
        if 'x' not in dir(self):
            self.x = SMTExplainer(self.enc, self.intvs, self.imaps,
                                  self.ivars, self.feature_names, self.num_class,
                                  self.options, self)
        all_expl = self.x.compute_minimal_coalitions(feature_ids)
        return all_expl
