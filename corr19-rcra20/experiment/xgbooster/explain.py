...
class SMTExplainer(object):
    ...
    def is_minimal(self,c_str,minimal_coalitions):
        minimals = list(minimal_coalitions.keys())
    ...
    def compute_all_minimal_expls(self, sample):
        ...
        for f_id in self.rhypos:
            coalition = [f_id]
            c_str = self.convert_coalition_to_str(coalition)

            if self.oracle.solve([self.selv] + coalition) is False:
                minimal_coalitions[c_str] = len(coalition)
            else:
        ...
        while queue:
            ...
            for f_id in self.rhypos:
                if f_id not in c:
                    c_new = c + [f_id]
                    c_str = self.convert_coalition_to_str(c_new)
                    if visited.get(c_str) is None:

                        if ((self.oracle.solve([self.selv] +c_new) is False) and (self.is_minimal(c_str, minimal_coalitions) is True)):
                            minimal_coalitions[c_str] = c_new
                        else:
                            queue.append(c_new)
                        visited[c_str] = True
        expls = list(minimal_coalitions.values())
        ...
        for expl in all_expls:
            ...
            interpretable_expls.append(preamble)
